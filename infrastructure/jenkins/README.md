...
1. Build and start the Jenkins controller:
   ...
   This builds `space-invaders-jenkins` from the `Dockerfile` (`jenkins/jenkins:lts-jdk17` + `python3`/`pip` for the `Backend Tests`/`Run API Tests` stages, `nodejs`/`npm` for the `Frontend Build` stage, and the Docker CLI/Compose plugin for the `docker build`/`docker compose` stages — all run directly on the controller) and starts two containers:
...
- **`docker: command not found` in a `Build * Image` / `docker compose up` / `Shutdown` stage, or `npm: not found` in `Frontend Build`**: the image was built before that tooling was added to the `Dockerfile`, or the build cache served a stale layer — rebuild with `docker compose build --no-cache`.
